...
Source: academy-agents/academy  examples/08-discussion/group_chat_agents.py
        https://github.com/academy-agents/academy/blob/main/examples/08-discussion/group_chat_agents.py

Byte-for-byte upstream: these agent classes take the LLM as a constructor argument
(``llm_or_agent`` / ``llm=``), so NO edit is needed here at all -- the run harness
(solution.py / exercise.py) injects a local HuggingFace chat model via
``make_chat_model()`` and the Flowcept plugins wrap the code unchanged.
"""
