"""Vendored from the upstream Academy example (agent logic only).

Source: academy-agents/academy  examples/07-mol-design/mol_design_agents.py
        https://github.com/academy-agents/academy/blob/main/examples/07-mol-design/mol_design_agents.py

This is the REAL-chemistry example: GFN2-xTB ionization-energy calculations
(rdkit + ASE + xtb) driven by an LLM reasoning campaign built as a LangGraph
StateGraph inside an Academy ``@loop``. Nothing is mocked -- ``_compute_energy_sync``
runs an actual xTB relaxation and returns a real energy difference.
