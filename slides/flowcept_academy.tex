...
source exercises/aurora/env.sh   # frameworks; conda activate; HF offline; settings

# then just submit -- no editing needed
cd exercises/aurora/06-llm && qsub submit.pbs   # -> runs/06-llm_<date-time>/
\end{lstlisting}
  \small \texttt{install.sh aurora} clones the ALCF \texttt{frameworks} base env
  (so \texttt{xtb-python} for 07 installs); nothing is downloaded. All LLM usage reads
  ALCF's staged hub at \texttt{/flare/datasets/model-weights} --- \texttt{env.sh} points
  \texttt{HF\_HOME} there and sets \texttt{HF\_HUB\_OFFLINE=1}, so compute nodes need no
  network. \textbf{06/07/08} serve \texttt{Llama-3.1-8B-Instruct} on the node's own GPUs
  via vLLM (\texttt{FLOWCEPT\_USE\_VLLM=0} falls back to the same weights on CPU for
  06/08; 07 needs vLLM or Argo for tool calls). Provenance stays offline --- inspect
  with the same terminal tools, no images to shuttle around.
\end{frame}
