...
\subtitle{A 1-hour, hands-on tutorial --- CPU-friendly, Argo/OpenAI/local LLM}
...
exercises/
  ...
  aurora/   run on ALCF Aurora              (qsub submit.pbs; shared env.sh)
    <id>/   exercise.py  solution.py  README.md  [submit.pbs]
\end{lstlisting}
  \begin{itemize}\small
...
\begin{frame}{LLM backend: Argo $\to$ OpenAI $\to$ local CPU model}
  \begin{itemize}
  ...
  \texttt{FLOWCEPT\_TUTORIAL\_LLM=argo|openai|local}. Every call is recorded as an
...
\end{lstlisting}
  ...
  \end{itemize}
  \small The LLM is injected via \texttt{make\_chat\_model()} (Argo $\to$ OpenAI $\to$
  local) exactly where upstream builds \texttt{ChatOpenAI} --- \alert{zero other agent-code changes}.
\end{frame}
...
\begin{frame}[fragile]{Running on Aurora (CPU, offline, terminal-only)}
  Each example under \texttt{exercises/aurora/<id>/} has its own \texttt{submit.pbs};
  all share \texttt{exercises/aurora/env.sh} (modules + conda env + offline LLM + settings).
\begin{lstlisting}[style=sh]
# once, on a login node (has internet): build the env + pre-cache the local model
...
# then submit an exercise (edit -A <project> first)
cd exercises/aurora/06-llm && qsub submit.pbs   # -> runs/06-llm_<date-time>/
\end{lstlisting}
  \small \texttt{env.sh} sets \texttt{HF\_HUB\_OFFLINE=1} +
  \texttt{FLOWCEPT\_TUTORIAL\_LLM=local}, so compute nodes need no network (or use
...
\begin{frame}{Recap}
  \begin{itemize}
    ...
    \item \textbf{LLM:} Argo $\to$ OpenAI $\to$ local CPU model.
