...
\title{Capturing Provenance of Academy Examples with Flowcept}
\subtitle{A 1-hour, hands-on tutorial --- CPU-friendly, Argo/vLLM/OpenAI/local LLM}
...
\begin{frame}{What we'll do}
  Take the \textbf{stock Academy examples}, instrument each with \textbf{Flowcept}
  ...
  \begin{tabular}{r l}
    ...
    \midrule
    0 & Why provenance --- and why it matters for \emph{agentic} systems \\
    1 & Academy in a nutshell (reminder) \\
    2 & Flowcept + examples 1, 2, 3 --- and how to inspect provenance \\
    3 & Advanced: agents \& LLMs (examples 6, 8) \\
    4 & Extra advanced: LangGraph + real xTB (7) + Parsl (5), and failures \\
    \bottomrule
  \end{tabular}
  \vspace{0.6em}\\
...
\begin{frame}[fragile]{How the tutorial is laid out}
  Eight stock Academy examples, each a \textbf{step-by-step exercise}. As shipped
...
\begin{frame}{Why it matters \emph{more} for agentic systems}
  ...
  \vspace{0.4em}
  \begin{itemize}\small
    \item \textbf{Non-determinism.} LLM calls sample --- the same prompt yields
      different outputs. You \emph{cannot} reproduce a run without recording the
      actual prompt, response, model, and tokens.
    \item \textbf{Autonomy \& emergence.} \texttt{@loop}s act on their own; agents
      ...
      declared --- capture is the only way to know the real path taken.
    \item \textbf{Distribution across boundaries.} Work spans agents, processes,
      nodes, and \emph{frameworks} (Academy + LangGraph + Parsl). Lineage that
      crosses those boundaries is invisible without provenance.
    \item \textbf{Opaque reasoning.} Prompts, tool calls, and delegations bury the
      ``why.'' Provenance keeps every prompt/response --- the audit trail.
  ...
  \end{itemize}
  \centering\small\alert{Agents are non-deterministic, autonomous, and
  distributed --- exactly the case provenance was made for.}
\end{frame}

% =====================================================================
\section{Part 1 --- Academy, a reminder}

\begin{frame}{Academy in a nutshell}
  A framework for \textbf{distributed multi-agent systems} for science.
  \begin{itemize}
  ...
  \end{itemize}
  \vspace{0.4em}
  Scales from local threads $\to$ processes $\to$ Globus Compute / Parsl across nodes.
\end{frame}
...
% =====================================================================
\section{Part 2 --- Flowcept + examples 1, 2, 3}

\begin{frame}{Flowcept: provenance, one line to capture}
  \textbf{Flowcept} records \emph{what ran, with what inputs/outputs, when, and how
  it connects} --- the provenance graph. Its \emph{agentic} plugin patches Academy's
  \texttt{Runtime}, so capture is automatic.
  \vspace{0.4em}
  \begin{columns}[T]
    \begin{column}{0.52\textwidth}
      \small Model: \texttt{campaign} $\to$ \texttt{workflow} $\to$ \texttt{task}
      (action / loop / llm\_call), with \texttt{used}/\texttt{generated} and
      \texttt{parent\_task\_id} lineage.
    \end{column}
    \begin{column}{0.48\textwidth}
      \begin{exampleblock}{Zero agent-code changes}\small
        Wrap the run; the plugin does the rest.
      \end{exampleblock}
    \end{column}
  \end{columns}
\end{frame}
...
% =====================================================================
\section{Part 3 --- Advanced: agents \& LLMs}
...
% =====================================================================
\section{Part 4 --- Extra advanced \& failures}
...
* llm_call model=gpt-4o-2024-11-20  parent=tool_calling
\end{lstlisting}
\end{frame}
...
\end{lstlisting}
  \small You see \emph{which} molecule the LLM invented, that xTB rejected it, and
  that the successful runs still gave real energies (13.20, 14.12 eV) --- the
  debugging payoff, and the reason agentic runs need this most.
\end{frame}

\begin{frame}[fragile]{Running on Aurora (offline, terminal-only)}
  Each example under \texttt{exercises/aurora/<id>/} has its own \texttt{submit.pbs}
  (\texttt{-A ATPESC2026} already set); all share \texttt{exercises/aurora/env.sh}
  (modules + conda env + offline LLM + settings).
\begin{lstlisting}[style=sh]
# once, on a login node. Env lives in project space. No model download.
export FLOWCEPT_ENV_PREFIX=/lus/flare/projects/ATPESC2026/prov/envs/flowcept-academy
bash setup/install.sh aurora     # clones the frameworks base + delta (no LLM download)
source exercises/aurora/env.sh   # frameworks; conda activate --stack; HF offline

# then just submit -- no editing needed (submit.pbs brings vLLM up on the node)
cd exercises/aurora/06-llm && qsub submit.pbs   # -> runs/06-llm_<date-time>/
\end{lstlisting}
  \small \texttt{install.sh aurora} stacks a small delta on the ALCF \texttt{frameworks}
  module (which ships the tuned XPU \texttt{vllm}); nothing is downloaded. All LLM usage
  reads ALCF's staged hub at \texttt{/flare/datasets/model-weights} --- \texttt{env.sh}
  points \texttt{HF\_HOME} there and sets \texttt{HF\_HUB\_OFFLINE=1}, so compute nodes
  need no network. \textbf{vLLM is the only LLM backend on Aurora} ---
  \textbf{06/07/08}'s \texttt{submit.pbs} serve \texttt{Llama-3.1-8B-Instruct} on the
  node's own GPUs via \texttt{source ../vllm\_serve.sh \&\& vllm\_start}. \alert{No CPU
  fallback}: \texttt{FLOWCEPT\_USE\_VLLM=0} means you must use Argo instead
  (\texttt{ARGO\_USER=...}). Provenance stays offline --- inspect with the same terminal
  tools, no images to shuttle around.
\end{frame}

\begin{frame}[fragile]{vLLM on Aurora: serve it yourself (+ interactive)}
  \texttt{vllm\_start} does more than launch a server. On Aurora's XPU, \texttt{vllm
  serve} normally \alert{SIGSEGVs} inspecting the model architecture (a subprocess
  crash surfaced as ``\texttt{architectures ['LlamaForCausalLM'] failed to be
  inspected}''). We dodge it: \texttt{vllm\_start} first \textbf{primes the modelinfo
  cache in-process} (loads the model \emph{class}, no weights, no subprocess), then
  serves against the cache --- \alert{handled automatically}, nothing to do by hand.
  \vspace{0.3em}
  \begin{itemize}\small
    \item \textbf{Adopt-on-source}: sourcing \texttt{vllm\_serve.sh} in a \emph{fresh}
      shell detects an already-running server and re-exports \texttt{VLLM\_BASE\_URL}
      --- so \texttt{query.py --ask} routes to it without relaunching.
    \item \textbf{Context window}: \texttt{VLLM\_MAX\_MODEL\_LEN=32768} (8192 was too
      tight --- 08's group chat accumulates the whole discussion and overflowed it).
  \end{itemize}
\begin{lstlisting}[style=sh]
