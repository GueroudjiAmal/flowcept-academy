...
\begin{lstlisting}[style=sh]
# once, on a login node (has internet). Env lives in project space.
...
bash setup/install.sh aurora     # CLONES the frameworks base + pre-caches CPU LLM
...
\end{lstlisting}
  ...
  (so \texttt{xtb-python} for 07 installs) and caches the CPU model while online;
  \texttt{env.sh} sets \texttt{HF\_HUB\_OFFLINE=1}, so compute nodes need no network.
  \textbf{06/07/08} serve a real model on the node's own GPUs via vLLM
  (\texttt{FLOWCEPT\_USE\_VLLM=0} falls back to the CPU model for 06/08; 07 needs vLLM
  or Argo for tool calls). Provenance stays offline --- inspect with the same terminal
